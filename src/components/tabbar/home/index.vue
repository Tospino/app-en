...
<template>
	...
	<div class="home">
		...
		<scroll class="bscroll-wrapper" ref="wrapper" :data="recordGroup" :pullup="pullup" @pullup="_pullup" :pulldown="pulldown"
		 @pulldown="_pulldown" :scrollX="true">
			<div>
				<div class="commodity-swipe" v-if="topBananerList.length > 0">
					<van-swipe @change="onChange" :stop-propagation="false" :autoplay="2000" :initial-swipe="swipeIndex">
						<van-swipe-item v-for="(banner, index) in topBananerList" :key="index" @click="swipeClick(banner, index)">
							<div class="w1">
            ...
            <van-button type="primary" size="large" class="jump-btn" @click="jumpRouter('限时抢购')">
              ...
              <span class="jump-btn-icon"></span>
				...
				<div class="good-recommend box">
					...
					<div class="flash-sale-2">
						<div class="pictures">
							<div class="p1" v-for="(finework, index) in homeObj.producteFineWorkpro" :key="finework.skuId">
								<img v-lazy="$webUrl + finework.imgUrl" @click="toDetail(finework.skuId, finework, 'finework', index)" />
								<div class="good-name clamp-2">
									<span v-if="finework.activityType == 1" class="clamp_clear" :class="{
                      clear_sale:
				...
				<div class="good-Clearance box" v-if="clear_list.length != 0">
					...
					<div class="flash-sale-2">
						<div class="flex goods_items flex_start" v-for="(item, indx) in clear_list" :key="indx" @click="
                $router.push({
                  name: '商品详情',
                  query: {
                    skuId: item.skuId,
              ...
              ">
							...
							<img src="@/assets/img/tabbar/home/clearsale/saleout.png" class="goods_sale" v-show="item.activityState === 2"
							 alt srcset />
							<div class="flex_col w100">
								<div class="good-name line2">{{ item.skuName }}</div>
								<div class="flex_end">
									<span class="goods_discount" :class="{ pre_fb: item.activityState === 0 }">
										<!-- parseInt((1 - item.activityPrice / item.salePrice) * 100) -->
										{{ item.percent }}% off
									</span>
								</div>
								<div>
									<span class="goods_price" :class="{ pre_fc: item.activityState === 0 }">{{ jn }}{{ item.activityPrice }}</span>
									<span class="goods_dis_price">{{ jn }}{{ item.salePrice }}</span>
								</div>
								<span class="goods_btn flex_center2" v-if="item.activityState === 0">Coming Soon</span>
				...
				<div class="good-world box">
					...
					<div class="good-world-brand">
						<div class="brand-p-1">
							<img v-lazy="$webUrl + brandLogo1.brandLogo" class="brand-p-1-left" @click="toSearOne(brandLogo1.brandId, 'brandId', 10)" />
							<img v-lazy="$webUrl + brandLogo2.brandLogo" class="brand-p-1-right-top" @click="toSearOne(brandLogo2.brandId, 'brandId', 11)" />
							<img v-lazy="$webUrl + brandLogo3.brandLogo" class="brand-p-1-right-bottom" @click="toSearOne(brandLogo3.brandId, 'brandId', 12)" />
						</div>
						<div class="brand-p-2">
							<img v-lazy="$webUrl + globalPro.brandLogo" v-for="(globalPro, index) in globalProList" :key="index" @click="toSearOne(globalPro.brandId, 'brandId', index)" />
						</div>
					</div>
				</div>
				<div class="good-world-best">
					...
					<div class="flash-sale-2">
						<div class="pictures">
							<div class="good-world-best-p1" :class="'cximg' + (index + 1)" v-for="(fineSale1, index) in fineSaleList1" :key="index">
								<img v-lazy="$webUrl + fineSale1.imgUrl" @click="
				...
				<div class="exhibition">
					<div class="flash-sale-2">
						<div class="pictures">
							<div class="p1" v-for="(fineSale2, index) in fineSaleList2" :key="fineSale2.skuId">
								<img v-lazy="$webUrl + fineSale2.imgUrl" @click="
				...
				<div class="good-popular box">
					...
					<div class="good-popular-top">
						<div class="good-popular-top-1" v-for="(category, index) in homeObj.producteFinecategory" :key="category.categoryId"
					...
					<div class="footer-exhibition">
							...
							<div class="exhibition-left" v-for="(searchgoodDao, index) in searchgoodDaolist" :key="index">
								<div class="exhibition-img" @click="toDetail(searchgoodDao.skuId, searchgoodDao)">
									<div class="shouwan" v-if="!searchgoodDao.canSalesNum">
										<!-- Out of Stock -->
										<img src="@/assets/img/tabbar/home/clearsale/saleout.png" class="goods_sale" v-if="
                        ...
                        searchgoodDao.locationNameEng
                          ...
                          : ""
								...
								<div class="clamp-2 miaoshu">
									...
									<span v-if="searchgoodDao.activityType == 1" class="clamp_clear" :class="{
                      clear_sale:
                      ...
                      searchgoodDao.discountPrice
                        ...
                        : searchgoodDao.salePrice
									...
									<span class="fl-right" style="color: red" v-show="searchgoodDao.skuSalesNum">
										Sales:{{
...
<script>
	...
	import {
		getuserinfoApi
	} from "@/api/accountSettings/index";
	import {
		couponDrawApi
	...
	export default {
		...
		data() {
			return {
				...
				// 圣诞
				christmas_home_bg: {
					backgroundImage: 'url(' + require('../../../assets/img/activity/christmas/christmas_home_labe.png') + ')',
					backgroundRepeat: 'no-repeat',
					backgroundSize: '100% 100%'

				},
				christmas_home_top: {
					backgroundImage: 'url(' + require('../../../assets/img/activity/christmas/christmas_top.png') + ')',
					backgroundRepeat: 'no-repeat',
					backgroundSize: '100% 100%'
