  ...
  <div class="recharge-flow">
    ...
    <div v-if="billList.length > 0">
      ...
      <div class="flow-con">
        <div
          class="flow-bill"
          v-for="(billItem, index) in billList"
          :key="index"
          @click="toDetail(billItem.prepaidId)"
        >
          <div class="bill-p1">
          ...
          <div class="bill-p1">
            <div class="fl-left">
              Actual transfer:GHS {{ billItem.prepaidMoney }}
            </div>
            <div class="fl-right">
              {{ orderStatus(billItem.paymentStatus, "paymentStatusList") }}
            </div>
          </div>
          <div class="bill-p2">
            <span>{{ billItem.createTime }}</span>
            <div
              class="del-btn fl-right"
              @click.stop="deleteItem(billItem.prepaidId)"
            >
              Delete
            </div>
          </div>
