<template>
	...
	<div class="account-balance">
		<balance-header title="Balance" title2="Records" @jumpRouter="jumpRouter" jumpName="账户明细"></balance-header>
		<div class="balance-bottom">
			...
			<div class="money-list">
				<div v-for="(prepaidMoney, index) in prepaidMoneyList" :key="index" :class="{ active: index == currentIndex }"
				 @click="choiceItem(prepaidMoney, index)">
					<span class="mony1">{{ prepaidMoney.money }}&nbsp;GHS</span>
					<br />
					<span class="mony2">To the account: GHS {{ prepaidMoney.arrivalAmount }}</span>
				</div>
			</div>
			...
			<div v-if="customMony>0" class="btn-next" @click="showpaymen" :style="{ backgroundColor: disabledSubmit ? '#FA5300' : '#999' }">
				Next
			</div>
			<div v-else class="btn-next" :style="{ backgroundColor: disabledSubmit ? '#FA5300' : '#999' }">
				Next
			</div>
		...
		<!-- 选择付款方式弹窗 -->
		<action-sheet-paymen ref="actionSheetPaymen" :moeny="Number(currentItem.money)" @paymoeny="paymoeny" @orderpay="orderpay"
		 :ifYuEStatus="true"></action-sheet-paymen>
