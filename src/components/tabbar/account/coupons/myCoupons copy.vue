  ...
  <div class="my-coupons">
    ...
    <div class="selection">
      ...
      <div class="selection-conten" v-else-if="active==1">
        <van-list class="activity" v-model="loading" :finished="finished" finished-text="loaded">
...
export default {
  ...
  methods: {
    onLoad() {
      // 监听底部达到了再次渲染列表
      this.userallCoupon(this.formData);
    },
    // 优惠券
    userallCoupon(total) {
      APPgetuserallCouponListApi(total).then((res) => {
        if (res.code == 0) {
          this.formData.page = this.formData.page + 1;
          // 列表数据懒加载
          if (res.Data.totalPage >= res.Data.currPage) {
            this.shopCoupon = this.shopCoupon.concat(res.Data.list);
          } else {
            this.loading = false;
            this.finished = true;
          }
    ...
    userDrawCoupon(active) {
      ...
      APPgetuserallcouponlistexpireApi(this.formData).then((res) => {
        if (active == 1) {
